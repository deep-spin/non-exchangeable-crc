...
def get_lhat_nonx(calib_loss_table, lambdas, alpha, W_submatrix, B=1):

    n = calib_loss_table.shape[0]

    valid_lhat = []
    
    # Compute N_w for all rows at once
    N_w = np.sum(W_submatrix, axis=1)
    # Compute the weighted sum using np.dot
    weighted_sum = np.dot(W_submatrix, calib_loss_table)
    # Compute rhat for all lambdas
    rhat = weighted_sum / N_w[:, np.newaxis]
    # Expand dimensions of N_w for broadcasting
    expanded_N_w = N_w[:, np.newaxis]
    alpha_check = ((expanded_N_w / (expanded_N_w + 1)) * rhat + B / (expanded_N_w + 1)) >= alpha
    lhat_idx = np.maximum(np.argmax(alpha_check, axis=1) - 1, 0)  # Compute lhat_idx for all rows at once
    valid_lhat = [lambdas[idx] for idx in lhat_idx]
...
def compute_trial(loss_table, size_table, lambdas, W, alpha, num_calibration):
    ...
    # Do the same thing with the weights
    W = W[perm][:, perm]
    calib_indices = np.arange(num_calibration)
    valid_indices = np.arange(num_calibration, len(loss_table))
    W_submatrix = W[valid_indices][:, calib_indices] # (1110, 2500) checked

    # Note: in standard CRC you get one lhat for all examples in the validation set as follows:
    # Compute threshold.
    ...
    # This is different for non-X CRC: lhat depends on the specific test example
    lhats = get_lhat_nonx(calib_loss_table, lambdas, alpha, W_submatrix)
    avg_loss = [ 
        valid_loss_table[i, np.argmax(lambdas == lhats[i])]
        for i in range(len(valid_size_table))
    ]
    avg_loss = np.array(avg_loss).mean()
    avg_size = [ 
        valid_size_table[i, np.argmax(lambdas == lhats[i])]
        for i in range(len(valid_size_table))
    ]
    avg_size = np.array(avg_size).mean()

    return lhats, avg_loss, avg_size


def run_trials(scores, losses, W, alpha, num_trials, num_calibration=3000, max_lambdas=10000):
    ...
    for i in tqdm.tqdm(range(num_trials), desc="Running trials"):
        lhats, risk, size = compute_trial(loss_table, size_table, lambdas, W, alpha, num_calibration)
        results.append(pd.DataFrame(dict(risk=risk, size=size, alpha=alpha), index=[i]))
        all_risk.append(risk)
        all_lhat.append(np.array(lhats).mean())
...
def plot_histograms(df, alpha, output_dir, num_trials, num_calibration):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"{num_trials}_{num_calibration}_{alpha}_qa_histograms_nonX.pdf"))
    print(f"The mean and standard deviation of the risk over {len(df)} trials "
          f"are {df['risk'].mean()} and {df['risk'].std()} respectively.")
    print(f"The mean and standard deviation of the size over {len(df)} trials "
          f"are {df['size'].mean()} and {df['size'].std()} respectively.")
    
    np.save(os.path.join(output_dir, f"{num_trials}_{num_calibration}_{alpha}_qa_nonX_risk"), risk)
    np.save(os.path.join(output_dir, f"{num_trials}_{num_calibration}_{alpha}_qa_nonX_size"), size)
...
def main(args):
    ...
    W = np.load("results/w-normalized-multi-qa-mpnet-base-dot-v1.npy")
    ...
    plot_histograms(df, args.alpha, args.output_dir, args.num_trials, args.num_calibration)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_calibration", type=int, default=3000) # 2500
