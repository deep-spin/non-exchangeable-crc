...
# taken from https://github.com/aangelopoulos/conformal-risk/blob/main/core/get_lhat.py
...
def compute_trial(loss_table, size_table, lambdas, alpha, num_calibration):
    ...
    lhat = get_lhat(calib_loss_table, lambdas, alpha)
...
def plot_histograms(df, alpha, output_dir, num_trials, num_calibration):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"{num_trials}_{num_calibration}_{alpha}_qa_histograms.pdf"))
    ...
    np.save(os.path.join(output_dir, f"{num_trials}_{num_calibration}_{alpha}_qa_risk"), risk)
    np.save(os.path.join(output_dir, f"{num_trials}_{num_calibration}_{alpha}_qa_size"), size)
